...
- Next: **launch ops** (manual, not code) — create Render blueprint + Vercel project, fill prod secrets/CORS, set Paystack **live** keys + webhook URL (`/api/v1/payments/webhook`), DNS, Sentry DSNs, then smoke-test the full flow. Optional follow-ups: Sentry source-map upload (flip `@sentry/cli` to built + add auth token), analytics charts/time-series.
